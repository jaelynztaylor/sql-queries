...
SELECT * FROM invoices ORDER BY total < 10.00; --to get greater than, use the less than sign?? why?
...
SELECT * FROM invoices WHERE billing_city = 'Chicago' AND billing_state = 'IL' AND billing_country='USA' AND total > 5 GROUP BY total;
-- The billing addresses of the 5 most valuable invoices from Mountain View CA
SELECT * FROM invoices WHERE billing_city = 'Mountain View' AND billing_state = 'CA' AND billing_country = 'USA' GROUP BY total LIMIT 5;
-- Gotta reward those big spenders!

-- A list of the 10 most valuable invoices made before January 1st, 2010
SELECT * FROM invoices WHERE invoice_date < '2010-01-01' GROUP BY total ORDER BY total DESC LIMIT 10 ;
